...
const Chat = () => {
    ...
    const { mutate, isPending } = useMutation({
        ...
        onSuccess: (data) => {
            ...
            setMessages((prev) => setMessages([...prev, data]))
    ...
    const handleSubmit = (e) => {
        ...
        mutate(query)
        setMessages((prev) => setMessages([...prev, query]))
        setText('')
    }
        ...
        <div className="min-h-[calc(100vh-6rem)] grid grid-rows-[1fr,auto]">
            ...
            <form onSubmit={handleSubmit} className="max-w-4xl pt-12">
                <div className="join w-full">
                    ...
                    <button className="btn btn-primary join-item" disabled={isPending}>{isPending ? 'please wait' : 'ask question'}</button>
                </div>
            </form>
        </div>
    )
